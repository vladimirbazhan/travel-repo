define(['./module'], function(controllers) {
    ...
    function TripEditCtrl($scope, $routeParams, $location, Backend, Auth, Entity, Alerts, $http, $route) {
        ...
        vm.mapNavigateByTitle = mapNavigateByTitle;
        vm.stickPositionToggle = stickPositionToggle;
        ...
        function TripChangesSaver(vm) {
            this.save = function () {
                if (!vm.isMapStick) {
                    var mapInfo = {};
                    mapInfo.mapCenter = vm.map.getCenter();
                    mapInfo.mapZoom = vm.map.getZoom();
                    vm.trip.MapInfo = JSON.stringify(mapInfo);
                }
                if (vm.editMode) {
